...
const screenForNames = (arr) => {
  // Solution code here...
  let newarr = arr.filter(name => /^((Mr.)|(Mrs.)|(Ms.)|(Dr.))\s\w+$/.test(name));
  return newarr;
...
const toTitleCase = (arr) => {
  // Solution code here...
  let newarr =  arr.map(str => str.replace(str.charAt(0), str.charAt(0).toUpperCase()));
  return newarr;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newarr = arr.filter(person => person.mass > 77).map(person => person.name).join(' - ');
  return newarr;
};
